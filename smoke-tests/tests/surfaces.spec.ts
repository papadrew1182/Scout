/**
 * Smoke tests: surface visibility and role enforcement.
 *
...
test.describe("Adult surfaces", () => {
  ...
  test("grocery page loads", async ({ page }) => {
    ...
    // Should see grocery page content or empty state
...
test.describe("Child surfaces", () => {
  ...
  test("child does NOT see Accounts & Access", async ({ page }) => {
    ...
    const visible = await page.locator("text=Accounts & Access").isVisible();
