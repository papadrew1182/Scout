...
def create_note(family_id: uuid.UUID, payload: NoteCreate, db: Session = Depends(get_db)):
    return notes_service.create_note(db, family_id, payload)


@router.get("/{note_id}", response_model=NoteRead)
def get_note(family_id: uuid.UUID, note_id: uuid.UUID, db: Session = Depends(get_db)):
...
def archive_note(family_id: uuid.UUID, note_id: uuid.UUID, db: Session = Depends(get_db)):
    return notes_service.archive_note(db, family_id, note_id)


@router.post("/{note_id}/unarchive", response_model=NoteRead)
def unarchive_note(family_id: uuid.UUID, note_id: uuid.UUID, db: Session = Depends(get_db)):
    return notes_service.unarchive_note(db, family_id, note_id)


@router.delete("/{note_id}", status_code=204)
def delete_note(family_id: uuid.UUID, note_id: uuid.UUID, db: Session = Depends(get_db)):
