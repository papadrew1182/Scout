...
def list_overdue(family_id: uuid.UUID, db: Session = Depends(get_db)):
    return finance_service.list_overdue_bills(db, family_id)


@router.get("/unpaid", response_model=list[BillRead])
def list_unpaid(family_id: uuid.UUID, db: Session = Depends(get_db)):
    return finance_service.list_unpaid_bills(db, family_id)


@router.post("", response_model=BillRead, status_code=201)
def create_bill(family_id: uuid.UUID, payload: BillCreate, db: Session = Depends(get_db)):
    return finance_service.create_bill(db, family_id, payload)


@router.get("/{bill_id}", response_model=BillRead)
def get_bill(family_id: uuid.UUID, bill_id: uuid.UUID, db: Session = Depends(get_db)):
...
def pay_bill(family_id: uuid.UUID, bill_id: uuid.UUID, db: Session = Depends(get_db)):
    return finance_service.mark_bill_paid(db, family_id, bill_id)


@router.post("/{bill_id}/unpay", response_model=BillRead)
def unpay_bill(family_id: uuid.UUID, bill_id: uuid.UUID, db: Session = Depends(get_db)):
    return finance_service.mark_bill_unpaid(db, family_id, bill_id)


@router.delete("/{bill_id}", status_code=204)
def delete_bill(family_id: uuid.UUID, bill_id: uuid.UUID, db: Session = Depends(get_db)):
