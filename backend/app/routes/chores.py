...
def list_chore_templates(family_id: uuid.UUID, db: Session = Depends(get_db)):
    return chore_service.list_chore_templates(db, family_id)


@router.post("", response_model=ChoreTemplateRead, status_code=201)
def create_chore_template(family_id: uuid.UUID, payload: ChoreTemplateCreate, db: Session = Depends(get_db)):
    return chore_service.create_chore_template(db, family_id, payload)


@router.get("/{template_id}", response_model=ChoreTemplateRead)
def get_chore_template(family_id: uuid.UUID, template_id: uuid.UUID, db: Session = Depends(get_db)):
