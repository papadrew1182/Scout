...
def ai_chat(body: ChatRequest, db: Session = Depends(get_db)):
    require_family(db, body.family_id)
    require_member_in_family(db, body.family_id, body.member_id)
...
def daily_brief(body: BriefRequest, db: Session = Depends(get_db)):
    require_family(db, body.family_id)
    require_member_in_family(db, body.family_id, body.member_id)
    result = orchestrator.generate_daily_brief(db, body.family_id, body.member_id)
    return BriefResponse(**result)


@router.post("/plans/weekly", response_model=WeeklyPlanResponse)
def weekly_plan(body: WeeklyPlanRequest, db: Session = Depends(get_db)):
    require_family(db, body.family_id)
    require_member_in_family(db, body.family_id, body.member_id)
    result = orchestrator.generate_weekly_plan(db, body.family_id, body.member_id)
    return WeeklyPlanResponse(**result)


@router.post("/meals/staples", response_model=StapleMealsResponse)
def staple_meals(body: StapleMealsRequest, db: Session = Depends(get_db)):
    require_family(db, body.family_id)
    require_member_in_family(db, body.family_id, body.member_id)
    result = orchestrator.suggest_staple_meals(db, body.family_id, body.member_id)
...
def list_conversations(
    ...
    stmt = (
        ...
        .where(AIConversation.family_member_id == member_id)
