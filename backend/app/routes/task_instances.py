...
def generate_tasks(family_id: uuid.UUID, target_date: date = Query(...), db: Session = Depends(get_db)):
...
def get_task_instance(family_id: uuid.UUID, instance_id: uuid.UUID, db: Session = Depends(get_db)):
...
def mark_completed(family_id: uuid.UUID, instance_id: uuid.UUID, payload: TaskInstanceComplete, db: Session = Depends(get_db)):
...
def apply_override(family_id: uuid.UUID, instance_id: uuid.UUID, payload: TaskInstanceOverride, db: Session = Depends(get_db)):
...
def list_step_completions(family_id: uuid.UUID, instance_id: uuid.UUID, db: Session = Depends(get_db)):
